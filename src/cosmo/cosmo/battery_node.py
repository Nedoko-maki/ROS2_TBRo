...
class BatteryNode(Node):
    ...
    def _init_chip(self):  # initialising the IC chip on powerup.
        ...
        self._wait(0xDB, 0x8000, msg_2) # Poll ModelCFG.Refresh(highest bit) until it becomes 0 to confirm IC completes model loading.
    ...
    def get_battery_state(self):
        self.state["RepCap"] = self._read_register(0x05)  # Capacity in mAh  
        self.state["RepSOC"] = self._read_register(0x06)  # Capacity as a percentage, 256 levels. Round to nearest int. 
        # SOC means State of Charge. 
        self.state["TTE"] = self._read_register(0x11)  # Time to Empty value, each level is 5.25 seconds. 
        self.state["VFOCV"] = self._read_register(0xFB) # Open-Circuit Voltage, in volts apparently. No idea about the resolution. 
        self.state["SPPCurrent"] = self._read_register(0xDA) # Sustained peak current in mA 
        self.state["FullCapRep"] = self._read_register(0x10)  # Capacity in Ah
        self.state["DesignCap"] = self._read_register(0x18)  # Design capacity in Ah
